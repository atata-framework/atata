namespace Atata.WebDriver.IntegrationTests.Verification;
...
public class ShouldTests : WebDriverSessionTestSuite
{
    ...
    {
        var sut = Go.To<WaitingPage>();

        sut.ButtonWithoutWait.Click()
            .Result.Should.BePresent();
    }

    [Test]
    public void BePresent_AtOnce()
    {
        var sut = Go.To<WaitingPage>();
        ...
        AssertThrowsAssertionExceptionWithUnableToLocateMessage(() =>
            sut.Result.Should.AtOnce.BePresent());
    }
